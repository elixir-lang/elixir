...
translate_macro({Kind, Line, [Call]}, S) when Kind == def; Kind == defmacro ->
  record(Kind, S),
...
translate_macro({Kind, Line, [Call, KV]}, S) when Kind == def; Kind == defmacro ->
...
translate_macro({Kind, Line, [Name, Args, Guards, KV]}, S) when Kind == def; Kind == defmacro ->
  record(Kind, S),
  assert_module_scope(Line, Kind, S),
  case KV of
    [{do, Expr}] -> [];
...
translate_macro({Kind, Line, Args}, S) when is_list(Args), Kind == def; Kind == defmacro ->

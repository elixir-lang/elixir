...
-export([assert_behavior/2, owner_methods/1, inherit_methods/1, mixin_methods/1]).
-include("elixir.hrl").
...
mixin_methods(#elixir_slate__{module=Module}) ->
  convert_methods(Module:module_info(exports) -- erlang_compiled());
...
mixin_methods(Self) ->
  ...
  convert_methods(Mixin:module_info(exports) -- erlang_compiled()).

% Public in Erlang

assert_behavior(Module, Object) when is_atom(Module) -> 
  assert_behavior(Module:module_info(exports) -- erlang_compiled(), Object);
...
owner_methods(Name) when is_atom(Name) ->
  ...
  convert_methods(Public -- elixir_internal()).
...
% Keeps a list of all elixir internal methods that are never explicitly shown.
elixir_internal() ->
  [{'__elixir_exported__',2}].
...
% Keeps a list of all methods automatically generated by Erlang.
erlang_compiled() ->
  [{module_info,0},{module_info,1}].

% Convert methods from Erlang

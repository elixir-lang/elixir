...
Elixir's type system will infer the `drive/2` function expects a `%User{}` struct as input and will either return `{:ok, dynamic()}`, `{:error, :no_choice}`, or `{:error, :not_allowed}`.

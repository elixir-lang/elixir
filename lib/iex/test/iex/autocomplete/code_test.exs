...
defmodule IEx.Autocomplete.CodeTest do
  use ExUnit.Case, async: true

  alias IEx.Autocomplete.Code, as: Subject
  ...
  test "Elixir completion sublevel" do
    assert expand('IEx.Autocomplete.CodeTest.SublevelTest.') == {:yes, 'LevelA', []}
  end
  ...
  test "completion for functions added when compiled module is reloaded" do
    ...
    File.write!("Elixir.IEx.Autocomplete.CodeTest.Sample.beam", bytecode)
    assert {:docs_v1, _, _, _, _, _, _} = Code.fetch_docs(Sample)
    assert expand('IEx.Autocomplete.CodeTest.Sample.foo') == {:yes, '', ['foo/0']}
    ...
    assert expand('IEx.Autocomplete.CodeTest.Sample.foo') == {:yes, '', ['foo/0', 'foobar/0']}
  after
    File.rm("Elixir.IEx.Autocomplete.CodeTest.Sample.beam")
  ...
  test "completion for struct names" do
    ...
    assert expand('%IEx.Autocomplete.CodeTe') == {:yes, 'st.MyStruct{', []}
    assert expand('%IEx.Autocomplete.CodeTest.MyStr') == {:yes, 'uct{', []}

    eval("alias IEx.Autocomplete.CodeTest.MyStruct")
  ...
  test "completion for struct var keys" do
    eval("struct = %IEx.Autocomplete.CodeTest.MyStruct{}")

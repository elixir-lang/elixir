...
defmodule ExUnit.DiffTest do
  ...
  test "guards" do
    assert_diff((x when x == 0) = 0)
    assert_diff((x when x == 0 and is_integer(x)) = 0)
    assert_diff((x when x == 0 or x == 1) = 0)
    assert_diff((x when x == 0 when x == 1) = 0)
    assert_diff((x when one() == 1) = 0)

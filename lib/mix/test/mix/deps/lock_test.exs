...
defmodule Mix.Deps.LockTest do
  ...
  test "creates new lock and manifest files" do
    in_fixture "no_mixfile", fn ->
      ...
      assert File.regular? "_build/lib/sample/ebin/.compile.lock"
    end
  end
  ...
  test "does not touch manifest file there is no change" do
    in_fixture "no_mixfile", fn ->
      ...
      File.rm! "_build/lib/sample/ebin/.compile.lock"

      Mix.Deps.Lock.write [bar: :bat, foo: :bar]
      refute File.regular? "_build/lib/sample/ebin/.compile.lock"
    end
  end
end

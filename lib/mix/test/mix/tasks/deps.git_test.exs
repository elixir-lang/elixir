...
defmodule Mix.Tasks.DepsGitTest do
  ...
  test "get, update and clean git repos with compilation" do
    ...
    in_fixture "no_mixfile", fn ->
      ...
      assert File.exists?("_build/lib/git_repo/ebin/Elixir.GitRepo.beam")
      assert File.read!("mix.lock") =~ %r/"git_repo": {:git, #{inspect fixture_path("git_repo")}, "[a-f0-9]+", \[\]}/

      purge [GitRepo]
      File.touch!("_build/lib/git_repo/ebin/.compile.elixir", { { 2010, 4, 17 }, { 14, 0, 0 } })
      Mix.Task.clear
      ...
      assert_received { :mix_shell, :info, [^message] }
      refute File.exists?("_build/lib/git_repo/ebin/Elixir.Git.Repo.beam")
    end
  after
    Mix.Project.pop
  end
  ...
  test "recompiles the project when a deps change" do
    ...
    in_fixture "no_mixfile", fn ->
      Mix.Tasks.Deps.Get.run []
      message = "* Getting git_repo (#{fixture_path("git_repo")})"
      assert File.exists?("_build/lib/git_app/ebin/.compile.lock")
      assert_received { :mix_shell, :info, [^message] }
      ...
      Mix.shell.flush
      File.touch!("_build/lib/git_app/ebin/.compile.lock", { { 2020, 4, 17 }, { 14, 0, 0 } })
  ...
  test "does not recompiles dependencies if nothing changed" do
    ...
    in_fixture "no_mixfile", fn ->
      Mix.Tasks.Deps.Get.run []
      message = "* Getting git_repo (#{fixture_path("git_repo")})"
      assert File.rm("_build/lib/git_app/ebin/.compile.lock") == :ok
      assert_received { :mix_shell, :info, [^message] }

      Mix.Tasks.Deps.Compile.run ["git_repo"]
      refute_received { :mix_shell, :info, ["Compiled lib/a.ex"] }
      refute File.exists?("_build/lib/git_app/ebin/.compile.lock")
    end
  after
    purge [GitRepo, GitRepo.Mix]
    Mix.Project.pop

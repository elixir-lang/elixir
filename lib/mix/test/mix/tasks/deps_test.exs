...
defmodule Mix.Tasks.DepsTest do
  ...
  test "prints list of dependencies and their status" do
    ...
    in_fixture "deps_status", fn ->
      ...
      msg = "  could not find an app file at \"_build/dev/lib/noappfile/ebin/noappfile.app\""
      assert_received {:mix_shell, :info, [^msg <> _]}

      assert_received {:mix_shell, :info, ["* nosemver (deps/nosemver)"]}

      msg = "  the app file specified a non-Semantic Versioning format: \"0.7\""
      assert_received {:mix_shell, :info, [^msg <> _]}
    end
  end
  ...
  test "prints list of dependencies and their status, including req mismatches and custom apps" do
    ...
    in_fixture "deps_status", fn ->
      ...
      msg = "  could not find app file at \"_build/dev/lib/noappfile/ebin/apppath.app\""
      refute_received {:mix_shell, :info, [^msg <> _]}

      msg = "  could not find app file at \"_build/dev/lib/noappfile/ebin/noappfile.app\""
      refute_received {:mix_shell, :info, [^msg <> _]}
    end
  end
  ...
  test "checks list of dependencies and their status on failure" do
    ...
    in_fixture "deps_status", fn ->
      ...
      msg = "  could not find an app file at \"_build/dev/lib/noappfile/ebin/noappfile.app\""
      refute_received {:mix_shell, :error, [^msg <> _]}
    end
  end
  ...
  test "fails on diverged dependencies on get/update" do
    ...
    in_fixture "deps_status", fn ->
      ...
      msg = "  the dependency git_repo in mix.exs is overriding a child dependency"
      assert_received {:mix_shell, :error, [^msg <> _]}
  ...
  test "fails on diverged dependencies on check" do
    ...
    in_fixture "deps_status", fn ->
      ...
      msg = "  different specs were given for the git_repo app:"
      assert_received {:mix_shell, :error, [^msg <> _ = received_msg]}
  ...
  test "fails on diverged dependencies even when optional" do
    ...
    in_fixture "deps_status", fn ->
      ...
      msg = "  the dependency git_repo in mix.exs is overriding"
      assert_received {:mix_shell, :error, [^msg <> _]}
    end
  end
  ...
  test "checks if dependencies are using old scm version" do
    ...
    in_fixture "deps_status", fn ->
      ...
      manifest_data =
        :erlang.term_to_binary({:v2, {System.version(), :erlang.system_info(:otp_release)}, :scm})

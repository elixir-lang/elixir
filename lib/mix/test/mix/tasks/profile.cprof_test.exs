...
defmodule Mix.Tasks.Profile.CprofTest do
  ...
  test "profiles evaluated expression", context do
    in_tmp context.test, fn ->
      assert capture_io(fn -> Cprof.run(["-e", @expr]) end) =~
               ~r(String\.Chars\.Integer\.to_string\/1 *\d)
    end
  end
  ...
  test "profiles the script", context do
    in_tmp context.test, fn ->
      ...
      assert capture_io(fn -> Cprof.run([profile_script_name]) end) =~
               ~r(String\.Chars\.Integer\.to_string\/1 *\d)
    end
  end
  ...
  test "filters based on limit", context do
    in_tmp context.test, fn ->
      refute capture_io(fn -> Cprof.run(["--limit", "5", "-e", @expr]) end) =~
               ~r(:erlang\.trace_pattern\/3 *\d)
    end
  end

  test "filters based on module", context do
    in_tmp context.test, fn ->
      refute capture_io(fn -> Cprof.run(["--module", "Enum", "-e", @expr]) end) =~
               ~r(String\.Chars\.Integer\.to_string\/1 *\d)
    end
  end
  ...
  test "Module matching", context do
    in_tmp context.test, fn ->
      refute capture_io(fn -> Cprof.run(["--matching", "Enum", "-e", @expr]) end) =~
               ~r(String\.Chars\.Integer\.to_string\/1 *\d)
    end
  end
  ...
  test "Module.function matching", context do
    in_tmp context.test, fn ->
      refute capture_io(fn -> Cprof.run(["--matching", "Enum.each", "-e", @expr]) end) =~
               ~r(anonymous fn\/3 in Enum\.each\/2 *\d)
    end
  end

  test "Module.function/arity matching", context do
    in_tmp context.test, fn ->
      assert capture_io(fn -> Cprof.run(["--matching", "Enum.each/8", "-e", @expr]) end) =~
  ...
  test "warmup", context do
    in_tmp context.test, fn ->
      assert capture_io(fn -> Cprof.run(["-e", @expr]) end) =~ "Warmup..."

defmodule Mix.Local.Installer do
  ...
  Install types supported by `Mix.Local.Installer`.
    ...
    * `:project` - install the current Mix project's artifact
    * `:local` - install the artifact located at `path`
    * `:url` - install the artifact retrievable at `url`
    * `:fetcher` - build and install the artifact generated by the `dep_spec`

...
defmodule Kernel.ErrorsTest do
  ...
  test "invalid unquote" do
    assert_compile_error(["nofile:1", "unquote called outside quote"], ~c"unquote 1")
  end
  ...
  test "def defp clause change from another file" do
    assert_compile_error(["nofile:4", "def hello/0 already defined as defp"], ~c"""
    defmodule Kernel.ErrorsTest.DefDefmacroClauseChange do
  ...
  test "duplicate map keys" do
    assert_compile_error(["nofile:1", "key :a will be overridden in map"], """
      %{a: :b, a: :c} = %{a: :c}
    """)
  ...
  test "| outside of cons" do
    assert_compile_error(["nofile:1", "misplaced operator |/2"], "1 | 2")

...
defmodule Kernel.ErrorsTest do
  ...
  test "undefined function" do
    ...
    assert captured =~ "undefined function baz/2"
  ...
  test "function without definition" do
    ...
    assert captured =~ "nofile:2:"
    assert captured =~ "implementation not provided for predefined def foo/0"
    ...
    assert captured =~ "nofile:10:"
    assert captured =~ "implementation not provided for predefined def example/2"
  end
  ...
  test "guard without definition" do
    ...
    assert captured =~ "nofile:2:"
    assert captured =~ "implementation not provided for predefined defmacro foo/1"
  end
  ...
  test "invalid unquote" do
    assert_eval_raise CompileError, "nofile:1: unquote called outside quote", ~c"unquote 1"
  end
  ...
  test "function local conflict" do
    ...
    assert captured =~ "nofile:3:"
    assert captured =~ "imported Kernel.&&/2 conflicts with local function"
  end
  ...
  test "macro invoked before its definition" do
    ...
    assert captured =~ "nofile:2:"
    assert captured =~ "cannot invoke macro bar/0 before its definition"
    ...
    assert captured =~ "nofile:2:"
    assert captured =~ "cannot invoke macro bar/1 before its definition"
  end
  ...
  test "macro captured before its definition" do
    ...
    assert captured =~ "nofile:3:"
    assert captured =~ "cannot invoke macro is_ok/1 before its definition"
  end
  ...
  test "def defp clause change from another file" do
    assert_eval_raise CompileError, ~r"nofile:4: def hello/0 already defined as defp", ~c"""
    defmodule Kernel.ErrorsTest.DefDefmacroClauseChange do
  ...
  test "no macros" do
    assert_eval_raise CompileError, "nofile:2: could not load macros from module :lists", ~c"""
  ...
  test "module imported from the same module" do
    assert_eval_raise CompileError,
                      ~r"nofile:3: you are trying to use/import/require the module Kernel.ErrorsTest.ScheduledModule.Hygiene which is currently being defined",
                      ~c"""
                      defmodule Kernel.ErrorsTest.ScheduledModule do
                        defmodule Hygiene do
                          import Kernel.ErrorsTest.ScheduledModule.Hygiene
  ...
  test "@compile inline with undefined function" do
    captured = capture_compile_error(~c"defmodule Test do @compile {:inline, foo: 1} end")
    assert captured =~ "nofile:1:"
    assert captured =~ "inlined function foo/1 undefined"
  end

  test "invalid @dialyzer options" do
    captured =
      capture_compile_error(~c"defmodule Test do @dialyzer {:nowarn_function, {:foo, 1}} end")

    assert captured =~ "nofile:1:"
    assert captured =~ "undefined function foo/1 given to @dialyzer :nowarn_function"

    captured = capture_compile_error(~c"defmodule Test do @dialyzer {:no_opaque, {:foo, 1}} end")
    assert captured =~ "nofile:1:"
    assert captured =~ "undefined function foo/1 given to @dialyzer :no_opaque"
  ...
  test "@on_load attribute with undefined function" do
    captured = capture_compile_error(~c"defmodule UndefinedOnLoadFunction do @on_load :foo end")

    assert captured =~ "nofile:1:"
    assert captured =~ "undefined function foo/0 given to @on_load"
  end
  ...
  test "wrong kind for @on_load attribute" do
    ...
    assert captured =~ "nofile:1:"
    assert captured =~ "expected @on_load function foo/0 to be a function, got \"defmacro\""
  end
  ...
  test "function head with guard" do
    assert_eval_raise CompileError, "nofile:2: missing :do option in \"def\"", ~c"""
  ...
  test "invalid args for function head" do
    ...
    assert captured =~ ~r"nofile:2: Kernel.ErrorsTest.InvalidArgsForBodylessClause"
    assert captured =~ "only variables and \\\\ are allowed as arguments in function head."
  end
  ...
  defp capture_compile_error(string) do

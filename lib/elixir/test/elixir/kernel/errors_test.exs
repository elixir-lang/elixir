...
defmodule Kernel.ErrorsTest do
  ...
  test "struct access on body" do
    ...
    assert_eval_raise(CompileError, msg, code)
  end
  ...
  test "invalid unquote splicing in oneliners" do
    ...
    assert_eval_raise(ArgumentError, msg, code)
  end
  ...
  test "function local conflict" do
    ...
    msg = "nofile:3: imported Kernel.&&/2 conflicts with local function"
    assert_eval_raise(CompileError, msg, code)
  end
  ...
  test "macro with undefined local" do
    ...
    assert_eval_raise(UndefinedFunctionError, msg, code)
  end
  ...
  test "function import conflict" do
    ...
    msg = "nofile:3: function exit/1 imported from both :erlang and Kernel, call is ambiguous"
    assert_eval_raise(CompileError, msg, code)
  end
  ...
  test "def defmacro clause change" do
    ...
    assert_eval_raise(CompileError, "nofile:3: defmacro foo/1 already defined as def", code)
  end
  ...
  test "def defp clause change from another file" do
    ...
    assert_eval_raise(CompileError, ~r"nofile:4: def hello/0 already defined as defp", code)
  end
  ...
  test "internal function overridden" do
    ...
    msg = "nofile:2: cannot define def __info__/1 as it is automatically defined by Elixir"
    assert_eval_raise(CompileError, msg, code)
  end
  ...
  test "no macros" do
    ...
    assert_eval_raise(CompileError, "nofile:2: could not load macros from module :lists", code)
  end
  ...
  test "invalid macro" do
    ...
    msg = "nofile: invalid quoted expression: {:foo, :bar, :baz, :bat}"
    assert_eval_raise(CompileError, msg, code)
  end
  ...
  test "already compiled module" do
    code = 'Module.eval_quoted Record, quote(do: 1), [], file: __ENV__.file'
  ...
  test "characters literal are printed correctly in syntax errors" do
    ...
    assert_eval_raise(SyntaxError, "nofile:1: syntax error before: \"?す\"")
    ':ok ?す'
  end
  ...
  test "bodyless function with guard" do
    ...
    assert_eval_raise(CompileError, "nofile:2: missing :do option in \"def\"", code)
  end
  ...
  test "invalid args for bodyless clause" do
    ...
    msg = ~r"nofile:2: only variables and \\\\ are allowed as arguments in definition header."
    assert_eval_raise(CompileError, msg, code)
  end
  ...
  test "typespec errors" do
    ...
    assert_eval_raise(CompileError, "nofile:2: spec for undefined function omg/0", '''
  ...
  test "macros error stacktrace" do
    ...
    assert expected_stacktrace = rescue_stacktrace(code)
  end
  ...
  test "macros function clause stacktrace" do
    ...
    assert [{__MODULE__, :sample, 1, _} | _] = rescue_stacktrace(code)
  end
    ...
    expected_stacktrace = [
      {
        Kernel.ErrorsTest,
        :__before_compile__,
        [%Macro.Env{module: Kernel.ErrorsTest.MacrosCompiledCallback}],
        _
      }
      | _
    ]
  ...
  test "def fails when rescue, else or catch don't have clauses" do
    ...
    assert_eval_raise(CompileError, ~r"expected -> clauses for :else in \"def\"", code)
  end

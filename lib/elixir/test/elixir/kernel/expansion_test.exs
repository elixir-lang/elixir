...
defmodule Kernel.ExpansionTest do
  ...
  describe "remotes" do
    ...
    test "in matches" do
      message =
        ~r"cannot invoke remote function Hello.fun_that_does_not_exist/0 inside pattern matching"

      assert_raise CompileError, message, fn ->
        expand(quote(do: Hello.fun_that_does_not_exist() = :foo))
      end

      message = ~r"cannot invoke remote function :erlang.make_ref/0 inside pattern matching"

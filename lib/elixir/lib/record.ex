defmodule Record do
  ...
  considerably improving the quality of our software.
  ...
  Elixir also provides first-class support for exportable Records,
  which will we learn next.
  ...
  Notice that, different to `defrecordp`, `defrecord` expects a
  module as first argument. This is because `defrecord` is going
  to create a module named `User` with all the relevant. This module
  can then be imported and we can manipulate the user as with
  `defrecordp`:
  ...
  now longer shows a record as a tuple but in a formatted syntax.
  In fact, since working with external records is frequent, Elixir
  allows developers to skip importing the record completely in favor
  ...
  is no need to import the record first.
  ...
  All the functionality discussed aboved happens at compilation time.
  This means that both `user(user, age: 26)` and `User[user, age: 26]`
  discussed previously will be a simple tuple operation at compilation
  time.

  However, there are some situations where we want to set or update
  fields  dynamically and `defrecord` (and `defrecord` only) supports
  it out of the box:

  ...
  def make_overridable(module, tuples) when is_atom(module) and is_list(tuples) do
    ...
    :lists.foreach(
      fn
        ...
        other ->
          raise ArgumentError,
                "each element in tuple list has to be a {function_name :: atom, arity :: 0..255} tuple, " <>
                  "got: #{inspect(other)}"
      end,
      tuples
    )
  ...
  def make_overridable(module, behaviour) when is_atom(module) and is_atom(behaviour) do
    case check_module_for_overridable(module, behaviour) do
      ...
      {:error, error_explanation} ->
        raise ArgumentError,
              "cannot pass module #{inspect(behaviour)} as argument to defoverridable/1 " <>
                "because #{error_explanation}"
    end
  ...
  defp preprocess_attribute(key, value) when key in [:moduledoc, :typedoc, :doc] do
    case value do
      ...
      {line, doc} when is_integer(line) ->
        raise ArgumentError,
              "expected the #{key} attribute to contain a binary, a boolean, or nil, " <>
                "got: #{inspect(doc)}"

defmodule Module do
  ...
  *Note*: unlike `@after_compile`, the callback function/macro must
  ...
  defp simplify_signature({var, _, context}, acc) when is_atom(context) do
    var_string = Atom.to_string(var)
    if String.starts_with?(var_string, "_") do
      case String.trim_leading(var_string, "_") do
        "" ->
          autogenerated(acc, :arg)
        trimmed ->
          {{String.to_atom(trimmed), [], nil}, acc}
      end
    else
      {{var, [], nil}, acc}
    end
  ...
  defp expand_signature(key, {all_keys, acc}) when is_atom(key) do
    case previous_values(key, all_keys, acc) do
      {arg_position, acc} -> {{:"#{key}#{arg_position}", [], Elixir}, {all_keys, acc}}
  ...
  defp merge_signatures([signature_older | rest_older], [signature_newer | rest_newer], arg_position) do
    [ merge_signature(signature_older, signature_newer, arg_position) |
      merge_signatures(rest_older, rest_newer, arg_position + 1)
    ]
  end
  ...
  # merge_signature(older_signature, newer_signature, arg_position)
  # If third element in the tuple is an atom, it indicates its context precendence;
  # being `nil` higher, and `Elixir` lower.
  defp merge_signature({:\\, line, [left, right]}, newer, arg_position),
    do: {:\\, line, [merge_signature(left, newer, arg_position), right]}

  defp merge_signature(older, {:\\, _, [left, _]}, arg_position),
    do: merge_signature(older, left, arg_position)

  # The older signature, when given, always have higher context precedence,
  # nil precedence wins over Elixir
  defp merge_signature({_, _, nil} = older, _newer, _),
    do: older
  defp merge_signature(_older, {_, _, nil} = newer, _),
    do: newer
  defp merge_signature({_, _, Elixir} = older, _newer, _),
    do: older
  defp merge_signature(_older, {_, _, Elixir} = newer, _),
    do: newer

  # Both are a guess, so check if they are the same guess
  defp merge_signature({var, _, _} = older, {var, _, _}, _),
    do: older

  # Otherwise, returns a generic guess
  defp merge_signature({_, line, _}, _newer, arg_position),
    do: {:"arg#{arg_position}", line, Elixir}

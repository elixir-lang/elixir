defmodule Module do
  ...
  defp simplify_signature(signature) do
    {_, {:collect, acc}} = :lists.mapfoldl(&simplify_signature/2, {:collect, []}, signature)
    {signature, _}       = :lists.mapfoldl(&simplify_signature/2, {:generate, acc, acc}, signature)
    signature
  end
  ...
  defp autogenerated({:collect, acc}, key) do
    {:we_dont_care_about_this_yet, {:collect, [key|acc]}}
  end

  defp autogenerated({:generate, all_keys, acc}, key) do
    case previous_values(key, all_keys, acc) do
      {i, acc} -> {{:"#{key}#{i}", [], Elixir}, {:generate, all_keys, acc}}
      :none    -> {{key, [], Elixir}, {:generate, all_keys, acc}}
    end
  end

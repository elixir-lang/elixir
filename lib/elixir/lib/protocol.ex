...
defmodule Protocol do
  ...
  def __impl__!(protocol, for, env) do
    ...
    if for != Any and not Keyword.has_key?(built_in(), for) and for != env.module and
         for not in env.context_modules do
      cond do
        Code.ensure_compiled(for) != {:module, for} ->
          IO.warn(
        ...
        not function_exported?(for, :__struct__, 1) ->
          IO.warn(
            "you are implementing a protocol for #{inspect(for)} but said module does not provide a struct. " <>
              "Make sure the module name is correct and it contains a struct",
            env
          )

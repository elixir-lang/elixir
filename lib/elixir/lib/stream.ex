defmodule Stream do
  ...
  @doc """
  Returns a counter, either starting from offset, or given by function.
  ...
  ## Examples
      ...
      iex> Stream.counter() |> Enum.take(3)
      [0, 1, 2]

      iex> Stream.counter(1) |> Enum.take(3)
      [1, 2, 3]

      iex> Stream.counter(fn x -> x * 10 end) |> Enum.take(3)
      [0, 10, 20]
  """
  @doc since: "1.17.0"
  def counter(fun_or_offset \\ 0)

  def counter(offset) when is_integer(offset) do
    unfold(offset, &{&1, &1 + 1})
  end

  def counter(fun) when is_function(fun) do
    unfold(0, &{fun.(&1), &1 + 1})
  end
  ...
  See `counter/1` for built-in counters.

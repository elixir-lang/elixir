defmodule Kernel.ParallelCompiler do
  ...
  Returns either `{:ok, modules}` with the modules generated by each compiled file,
  or `{:error, errors}` with a list of `CompileError`s.
  """
  ...
  defp wait_for_messages(state) do
    ...
    receive do
      ...
      {:file_compiled, child_pid, file, {kind, reason, stack}} ->
        ...
        terminate(queued)
        {:error, [to_error(file, kind, reason, stack)]}
  ...
  defp handle_deadlock(waiting, queued) do
    deadlock =
      for {pid, _, file, _} <- queued do
        ...
        {file, on, struct(error, file: file)}
      end
    ...
    errors = for {_, _, error} <- deadlock, do: error
    {:error, errors}
  end
  ...
  defp to_error(file, kind, reason, stack) do
    CompileError.exception(
      ...
      description: Kernel.CLI.format_error(kind, reason, stack)
    )
  end
end

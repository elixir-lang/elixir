defmodule Kernel.SpecialForms do
  ...
  @doc """
  Evaluates to `not(left in right)`.

  See `Kernel.in/2` and `Kernel.not/1` for more information.

  ## Examples

      iex> :missing_value not in []
      true

      iex> :present_value not in [:present_value]
      false

  ## Special AST Considerations

  This is rewritten in the Elixir compiler, so `:left not in :right` appears
  ...
  The `not in/2` operator can be used in guard clauses as long as the
  ...
  translates to:
      ...
      when not(x >= 1 and x <= 3)

  """
  defmacro unquote(:"not in")(left, right), do: error!([left, right])
end

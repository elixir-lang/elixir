defmodule Access do
  ...
  ## Examples
      ...
      iex> list = [%{name: "john", salary: 10}, %{name: "francine", salary: 30}]
      iex> get_in(list, [Access.find(&(&1.name == "john")), :salary])
      10
      iex> get_and_update_in(list, [Access.find(&(&1.name == "john")), :salary], fn prev ->
      ...>   {prev, 15}
      ...> end)
  ...
  a key inside of a list:
      ...
      iex> list = [%{name: "john", salary: 10}, %{name: "francine", salary: 30}]
      iex> pop_in(list, [Access.find(&(&1.name == "francine"))])
      {%{name: "francine", salary: 30}, [%{name: "john", salary: 10}]}
      iex> pop_in(list, [Access.find(&(&1.name == "francine")), :name])
  ...
  specify defaults and safely traverse missing items.
      ...
      iex> list = [%{name: "john", salary: 10}, %{name: "francine", salary: 30}]
      iex> get_in(list, [Access.find(&(&1.name == "adam"), %{name: "adam", salary: 50}), :salary])
      50
      iex> get_and_update_in(list, [Access.find(&(&1.name == "adam"), %{name: "adam"}), :salary], fn prev ->
      ...>   {prev, 50}
      ...> end)
  ...
  An error is raised if the accessed structure is not a list:
      ...
      iex> get_in(%{}, [Access.find(&(&1.name == "john"))])
      ** (RuntimeError) Access.find/1 expected a list, got: %{}

  """

  @spec find((term -> boolean), term()) :: access_fun(data :: list, current_value :: list)
  ...
  a key inside of a list:
      ...
      iex> pop_in(list, [Access.find!(&(&1.name == "francine")), :name])
      {"francine", [%{salary: 30}, %{name: "john", salary: 10}]}
      iex> get_in(list, [Access.find!(&(&1.name == "adam")), :salary])

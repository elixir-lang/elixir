...
defmodule Enum do
  ...
  def filter_map(enumerable, filter, mapper) when is_list(enumerable) and is_function(filter, 1) do
    for item <- enumerable, filter.(item), do: mapper.(item)
  end

  def filter_map(enumerable, filter, mapper) when is_function(filter, 1) do

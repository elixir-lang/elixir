...
`Enum.filter_map/3`                              | [v1.5]        | `Enum.filter/2` + `Enum.map/2` or [`for`](`Kernel.SpecialForms.for/1`) comprehensions
...
`Kernel.to_char_list/1`                          | [v1.5]        | `to_charlist/1` (v1.3)
`as: true \| false` in `Kernel.SpecialForms.alias/2` and `Kernel.SpecialForms.require/2` | [v1.1]        | *None*
...
**EEx**                                          |               | 
`<%=` in middle and end expressions         | [v1.5]        | Use `<%` (`<%=` is allowed only on start expressions) (v1.0)
**IEx**                                          |               | 

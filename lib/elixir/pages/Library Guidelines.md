...
Every project has a `mix.exs` file, with instructions on how to build, compile, run tests, etc. Libraries commonly have a `lib` directory, which includes Elixir source code, and a `test` directory. A `src` directory may also exist for Erlang sources.
...
Elixir developers should avoid using exceptions for control-flow. For example, instead of:
...
One should prefer to:

```elixir
case File.read("some_path_that_may_or_may_not_exist") do
  {:ok, contents} -> # it worked
  {:error, _} -> # it failed
end
```

As a library author, it is your responsibility to make sure users are not required to use exceptions for control-flow in their applications. You can follow the same convention as Elixir here, using the name without `!` for returning `:ok`/`:error` tuples and the name with `!` for raising.
...
Developers should avoid using [the application environment](https://hexdocs.pm/elixir/Application.html#get_env/2) for configuring their own libraries. The application environment is **global** which means it becomes impossible for two dependencies to use your library in two different ways.
...
The solution is to provide configuration as close as possible to where it is used and not via the application environment. In case of a function, as keyword lists:
...
A library should not provide `use MyLib` functionality if all that `use MyLib` does is to `import`/`require` the module itself. For example, this is an anti-pattern:
...
it allows `use MyLib` to run *any* code into the `MyApp` module. For someone reading the code, it is impossible to assess the impact that `use MyLib` has in a module.

On the other hand, the following code is much clearer:
...
Although the previous section could be resumed as "avoid macros", both topics are important enough to deserve their own section.
...
Elixir developers should avoid spawning processes outside of a supervision tree, especially long-running ones. Instead, processes must be started inside supervision trees. This guarantees developers have full control over the initialization, restarts and shutdown of the system.
...
Each process started with the application must be listed as a child under the `Supervisor` above. We call those "static processes" because they are known upfront. For handling dynamic processes, such as the ones started during requests and other user inputs, one should look at the `DynamicSupervisor` module.

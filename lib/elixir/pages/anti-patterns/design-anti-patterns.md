...
config :dash_splitter_app,
  parts: 3

import_config "#{config_env()}.exs"
```

One of the functions implemented by the `DashSplitter` library is `split/1`. This function separates a string into a certain number of parts. The character used as a separator in `split/1` is always `"-"` and the number of parts is defined globally by the application environment. This value is retrieved by the `split/1` function by calling `Application.fetch_env!/2`, as shown next:

```elixir
defmodule DashSplitter do
  def split(string) when is_binary(string) do
    parts = Application.fetch_env!(:dash_splitter_app, :parts) # <= retrieve parameterized value
    String.split(string, "-", parts: parts)             # <= parts: 3
  end
end
```

As this parameterized value is global, all applications dependent on it can only split strings into the same number of parts. As configured above, this value is 3, as seen in these examples:
...
Depending on the library and use-case, different methods of configuration may be more appropriate. These could be in the form of keyword lists, as seen above, or configuration structs, initializing arguments passed at application startup, namespaced [behaviour implementations](https://hexdocs.pm/elixir/typespecs.html#behaviours), to name a few.

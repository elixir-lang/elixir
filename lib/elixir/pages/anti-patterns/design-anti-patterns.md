...
Side-effects are essential when writing production software. Reading a file, logging a message, sending messages between processes, etc. are all side-effects. Writing code with side-effects is not an anti-pattern. However, you must avoid scattering side-effects throughout your functions.

#### Example

Consider a functionality that must parse JSON files from disk, convert all keys to lowercase, and write the updated version to the same file. An initial implementation would look like this:
...
defmodule Recode do
  ...
  def recode!(file) do
    data = read_and_decode!(file)
    data = transform_data(data)
    encode_and_write!(file, data)
  end

  defp transform_data(data) do
    Map.new(data, fn {k, v} -> {String.downcase(k), v} end)
  end
...
defmodule Recode do
  ...
  def recode_contents!(binary) when is_binary(binary) do
    binary
    |> Jason.decode!()
    |> transform_data()
    |> Jason.encode!()

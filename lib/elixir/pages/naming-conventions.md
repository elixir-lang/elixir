...
Aliases, commonly used as module names, are an exception as they must be capitalized and written in `PascalCase`, like `OptionParser`. For aliases, capital letters are kept in acronyms, like `ExUnit.CaptureIO` or `Mix.SCM`.

Atoms can be written either in `:snake_case` or `:PascalCase`, although the convention is to use the snake case version throughout Elixir.

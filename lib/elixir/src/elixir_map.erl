...
expand_struct(Meta, Left, Right, #{context := Context} = E) ->
  ...
  case validate_struct(ELeft, Context) of
    ...
    false when Context == match ->
      compile_error(Meta, ?key(E, file), "expected struct name in a match to be a compile "
        "time atom, alias or a variable, got: ~ts", ['Elixir.Macro':to_string(ELeft)]);

    false ->
      compile_error(Meta, ?key(E, file), "expected struct name to be a compile "
...
validate_match_key(Meta, {Name, _, Context}, E) when is_atom(Name), is_atom(Context) ->
  ...
  compile_error(Meta, ?key(E, file), Message, [Name, Name]);
...
load_struct(Meta, Name, Args, InContext, E) ->
  ...
  of
    #{} = Struct ->
      Struct;
    Other ->
      compile_error(Meta, ?key(E, file), "expected ~ts.__struct__/~p to "
    ...
    error:undef ->
      ...
      case InContext andalso (?key(E, function) == nil) of
        ...
        false ->
          compile_error(Meta, ?key(E, file), "~ts.__struct__/~p is undefined, "
...
assert_struct_keys(Meta, Name, Struct, Assocs, E) ->
  [begin
     compile_error(Meta, ?key(E, file), "unknown key ~ts for struct ~ts",

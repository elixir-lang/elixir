...
module_define_behavior_test() ->
  F = fun() ->
    {bar,[]} = elixir:eval("module Foo; define_behavior('bar); __behavior__; end"),
...
module_behavior_defines_a_callback_module_test() ->
  F = fun() ->
    elixir:eval("module CallbackSpecificModule; define_behavior('bar); end"),
...
module_dispatching_to_callback_dispatch_to_ex_module_test() ->
  F = fun() ->
    elixir:eval("module Foo; define_behavior('bar); callbacks; def foo; 1; end; def bar(x, y); x - y - self.foo - foo; end; end"),
...
object_define_behavior_test() ->
  F = fun() ->
    {bar,[]} = elixir:eval("object Foo; def foo;1;end; define_behavior('bar); __behavior__; end"),
...
object_behavior_defines_a_callback_module_test() ->
  F = fun() ->
    elixir:eval("object CallbackSpecificObject; def foo; 1; end; define_behavior('bar); end"),
...
object_dispatching_to_callback_dispatch_to_ex_module_test() ->
  F = fun() ->
    elixir:eval("object Foo; define_behavior('bar); callbacks; def foo; 1; end; def bar(x, y); x - y - self.foo - foo; end; end"),

...
definition_test() ->
  F = fun() ->
    elixir:eval("module Foo::Bar::Baz, do: nil")
  end,
...
def_shortcut_test() ->
  F = fun() ->
    {1,[]} = elixir:eval("module Foo do\ndef version, do: 1\nend\nFoo.version")
  end,
  test_helper:run_and_remove(F, ['::Foo']).

macro_test() ->
  F = fun() ->
    {'::Foo',[]} = elixir:eval("module Foo do\ndef version, do: __MODULE__\nend\nFoo.version"),
...
macro_line_test() ->
  F = fun() ->
    ?assertMatch({2, []}, elixir:eval("module Foo do\ndef line, do: __LINE__\nend\nFoo.line")),
...
macro_file_test() ->
  F = fun() ->
    ?assertMatch({<<"nofile">>, []}, elixir:eval("module Foo do\ndef line, do: __FILE__\nend\nFoo.line")),
...
dynamic_ref_precedence_test() ->
  F = fun() ->
    elixir:eval("module A::Foo, do: def(l, do: A::Foo); module A::Bar, do: def(l(x), do: A::Bar)"),

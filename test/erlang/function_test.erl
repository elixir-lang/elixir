...
function_call_do_end_test() ->
  {[1,[{do,2},{else,3}]], _} = elixir:eval("x = fn(a, b, do: [a,b])\nx.(1) do\n2\nelse: 3\nend").

function_call_with_assignment_test() ->
  {3, [{a,_},{c, 3}]} = elixir:eval("a = fn(x, do: x + 2); c = a.(1)").

function_calls_with_multiple_expressions_test() ->
  {26, _} = elixir:eval("a = fn(a, b, do: a + b); a.((3 + 4 - 1), (2 * 10))").

function_calls_with_multiple_args_with_line_breaks_test() ->
  {5, _} = elixir:eval("a = fn(a, b, do: a + b); a.(\n3,\n2\n)").

function_calls_with_parenthesis_test() ->
  {3, [{a,_},{b,1}]} = elixir:eval("a = fn(x, do: x + 2).(b = 1)").
